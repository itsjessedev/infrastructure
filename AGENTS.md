...
This file is the Codex-facing operational mirror of [`CLAUDE.md`](/home/jesse/infrastructure/CLAUDE.md). Keep them aligned whenever repo-specific infrastructure duties change.
...
| File | Purpose |
|------|---------|
| [`docs/ssh-and-network-drives.md`](/home/jesse/infrastructure/docs/ssh-and-network-drives.md) | Complete SSH config, Samba, drive mappings, troubleshooting history |
| [`docs/devlab-process-management.md`](/home/jesse/infrastructure/docs/devlab-process-management.md) | OOM prevention, orphan cleanup, process watchdog, container memory limit |
| [`docs/unraid-logging.md`](/home/jesse/infrastructure/docs/unraid-logging.md) | Unraid persistent logging framework — log paths, monitor intervals, auto-cleanup |
| [`bin/claude-wrapper`](/home/jesse/infrastructure/bin/claude-wrapper) | Claude Code wrapper with wall detection, auto-restart, cleanup |
| [`claude-drop/main.py`](/home/jesse/infrastructure/claude-drop/main.py) | Claude Drop file sharing service (deployed to VPS) |
| [`reddit-relay/`](/home/jesse/infrastructure/reddit-relay) | Reddit relay service |
| [`system-utilities/`](/home/jesse/infrastructure/system-utilities) | Shared binaries, scripts, Claude commands |
...
- Unraid forwards ports `25831` (SSH) and `445` (SMB) from the Tailscale interface (currently `tailscale1`) to devlab
- Forwarding rules are in `start-devlab.sh` and recreated on container restart
- Subnet routes (`192.168.99.0/24`) are advertised but may need admin approval in the Tailscale console
...
Run all Unraid checks in parallel via SSH (`ssh -J desktop root@192.168.99.83`):
...
**6. SMART disk health:** Still check silently, but do NOT report unless critical (`SMART overall health = FAILED`, or pending sectors jump by 50+ since last check). Known baseline: `disk2` has 72 pending sectors as of `2026-03-14`. Disk space warnings are also suppressed because drives are pending replacement.
```bash
...
3. **Add them to the auto-cleanup** — update `~/bin/ssh-session-cleanup` (PAM hook) and/or `~/bin/process-health-check` (`KNOWN_BACKGROUND` allowlist if legitimate, or detection patterns if orphan-prone). Do NOT just kill them and move on. The same type of process must never appear as a buildup again.
...
The goal is that every time a new orphan/zombie type is discovered, the system learns to handle it. Over time, the auto-cleanup should become comprehensive.

**Key files:**
- `~/bin/ssh-session-cleanup` — PAM hook, kills all descendants on SSH disconnect
- `~/bin/process-health-check` — monitor, runs every 5 min, writes alerts to `/tmp/process-alerts.txt`
- `~/.config/container-startup.sh` — sets up PAM hook + monitors on container start
- Container memory limit: `12GB` (`--memory=12g` in `start-devlab.sh`)

## Local Documentation Rule

If you change a live infrastructure helper outside the repo (for example `~/bin/*`, `~/.config/container-startup.sh`, or Unraid-side scripts), update the corresponding repo docs in the same work block so this repo remains the written source of truth.

## Known Issues (Do NOT Nag About These)

- **Disk drives:** Pending sectors on `disk2` (72 as of `2026-03-14`), disks 1+2 nearly full, no parity. Jesse is aware. Only report if a drive's SMART overall health changes to `FAILED` or pending sectors spike dramatically (50+).
- **Overseerr container log:** ~290MB
- **Prowlarr container log:** ~365MB
- No Docker log rotation configured on Unraid yet.
